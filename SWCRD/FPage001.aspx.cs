...
public partial class SWCRD_FPage001 : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (ssUserName == "" || ssUserType !="04") { Response.Write("<script>alert('很抱歉，您尚未開通此功能權限，如有疑問請洽承辦單位或系統管理員聯絡洽詢。');location.href='../SWCDOC/SWC001.aspx';</script>"); }
    ...
    {
        //string pgSQL = " select g.*,s.GS004,s.GS005,s.GS006 from geouser g left join GuildShifts s on g.userid=s.GS002 and s.GS001=1 where Guild01=1; ";
        getGV2Date();
    ...
    private void getGV2Date()
    {
        ...
        string myGV2Data = " select A.*,E.LAND001 from swcswc A left join (select SWC000,MIN(LAND000) as LAND000 from TCGESWC.dbo.SWCLAND group by SWC000) D ON A.SWC00 = D.SWC000 left join (select SWC000,LAND000,LAND001 as LAND001 from TCGESWC.dbo.SWCLAND) E ON A.SWC00 = E.SWC000 and D.LAND000 = E.LAND000 where SWC04='審查中' and SWC022ID='" + ssUserGuild + "' " + pageSearchSql + " order by SWC00 desc; ";
        ConnectionStringSettings connectionString = ConfigurationManager.ConnectionStrings["TSLMSWCCONN"];
        using (SqlConnection TslmConn = new SqlConnection(connectionString.ConnectionString))
        {
            TslmConn.Open();
    ...
    protected void SaveRow_Click(object sender, EventArgs e)
    {
        ...
        using (SqlConnection dbConn = new SqlConnection(connectionString.ConnectionString))
        {
            ...
            using (var cmd = dbConn.CreateCommand())
            {
				...
				if(tmpSWC113 != "")
					...
					cmd.Parameters.Add(new SqlParameter("@SWC113", DBNull.Value));
				if(tmpSWC113_2 != "")
					cmd.Parameters.Add(new SqlParameter("@SWC113_2", tmpSWC113_2));
                else
					cmd.Parameters.Add(new SqlParameter("@SWC113_2", DBNull.Value));
        ...
        if (CBOX01.Checked)
        {
            ...
            //股長、管理者、承辦人員(SWC25)、承辦技師、審查委員(召集人、委員)、公會、義務人、聯絡人、章姿隆
            ...
            string exeSQLSTR2 = " select ETEmail as EMAIL from GuildGroup G Left Join ETUsers E On G.ETID = E.ETID where G.swc000 = '" + tmpSWC000 + "' and (ETEmail is not null and RGType = 'S3') ";
			exeSQLSTR2 += "UNION select ETEmail as EMAIL from ETUsers where ETID = '" + tmpSWC021ID + "' ";
            exeSQLSTR2 += "UNION ALL select email as EMAIL from tslm2.dbo.geouser g where ((department = '審查管理科' and ((jobtitle = '股長' and Tcgearea01 like '%" + tmpSWC012 + "%') or mbgroup02 = '系統管理員' or [name] = '" + tmpSWC025 + "' or [name] = '章姿隆')) or(userid = '" + tmpSWC022ID + "')) and status <> '停用' ";
        ...
        C20.swcLogRC("FPage001", "檢查會議發信通知", "目錄", "存檔", tmpSWC002 + "," + ssUserName);
