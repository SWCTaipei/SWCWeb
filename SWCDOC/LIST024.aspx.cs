...
public partial class SWCDOC_LIST024 : System.Web.UI.Page
{
    int ChkOverDay = 6;
    ...
    {
      //  Response.Write("< script type ='text / javascript'>  SearchChg();</ script >");
        string ssUserType = Session["UserType"] + "";//身分代碼
        string ssUserID = Session["ID"] + "";
        string ssUserName = Session["NAME"] + "";//使用者名稱
        ...
        if (!IsPostBack)//依照身分開啟功能
        {
            switch (ssUserType)
            {
				...
				case "09":
                    ...
                    if (ssUserID != ssUserGuild) { ChgUserType.Visible = true; DDLChange.Text = "公會"; }
    ...
    private void RemoveSelSession()//清除值
    {
    ...
    private void SessionSel2Page()
    {
        ...
        CHKSHTYPE.Checked = (Session["CHKSHTYPE"] + "" == "false");
    ...
    protected void GenerateDropDownList()//下拉選單項目增加
    {
    ...
    protected void NewSwc_Click(object sender, ImageClickEventArgs e)//跳頁到SWC002
    {
        Response.Redirect("SWC002.aspx?CaseId=ADDNEW");
    }
    protected void GetGVSWCList()//查詢下方表格資料並列出
    {
        ...
        C20.swcLogRC("LIST024", "水土保持申請案件", "目錄", "瀏覽", "");
    ...
    protected string limitListRange()
    {
        ...
        else
        {
            ...
            {
                case "01":
                    rValue = rValue + " AND SWC013ID = '" + ssUserID + "' ";
                    rValue = rValue + " AND SWC013TEL = '" + ssUserPW + "' ";
                    break;
                case "02":
            ...
            case DataControlRowType.DataRow:
                ...
                using (SqlConnection SwcConn = new SqlConnection(connectionString.ConnectionString))
                {
                    ...
                    while (readerSWC.Read())
                    {
                        ...
                        {
                            case "01":
                                if (tempSWC007 == "簡易水保" && (tempSWC004 == "退補件" || tempSWC004 == "受理中" || tempSWC004 == "申請中"))
                                {
                                    ShowEditBtn = "Y";
                                }
                                break;
    ...
    protected void RemoveSel_Click(object sender, EventArgs e)
    {
        ...
        DDLCadastralChange("01", DDLDistrict, DDLSection, DDLSection2, null);
        TXTNumber.Text = "";
        List02.Attributes["style"] = "visibility:hidden";
        // GetGVSWCList();
    }
    ...
    //  List01.Visible = false;
     ...
     /*  if (CHKSHTYPE.Checked)
        ...
        */
        Session["PrintExcelOdtStr"] = sqlSelectStr;
        List02.Attributes["style"] = "display：block";
    }
